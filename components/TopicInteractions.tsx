...
export default function TopicInteractions({
                                              topicId,
                                              initialStats,
                                          }: {
    topicId: string;
    ...
    async function loadAll() {
        ...
        try {
            const [detailRes, votesRes] = await Promise.all([
                ...
                fetch(`/api/topics/${topicId}/votes?limit=6`, { cache: "no-store" }), // ⬅ limit 6
            ]);
            if (detailRes.ok) {
    ...
    async function vote(value: 1 | -1) {
        if (!loggedIn) return signIn("discord");
        setSending(value);
        try {
            ...
            if (res.ok) {
                await loadAll(); // frissítjük statot + listát
            }
        } finally {
            setSending(null);
        }
            ...
            <div>
                <div className="flex items-center justify-between gap-3">
                    <div className="text-sm text-zinc-400">
                        <span className="font-medium text-zinc-200">{posPct}%</span> positive •{" "}
                        {stats.pos} up / {stats.neg} down • {stats.total} votes
                    </div>
                ...
                {loading ? (
                    ...
                    <ul className="divide-y divide-zinc-800">
                        {votes.slice(0, 6).map((v, idx) => {
                            const faded = idx === 5; // ⬅ a 6. elem (0-index: 5) legyen halvány
                            return (
                                <li
                                    key={v.id}
                                    className={`px-4 py-3 flex items-center gap-3 transition ${
                                        faded ? "opacity-50 grayscale-[30%]" : ""
                                    }`}
                                    aria-label={faded ? "Less emphasized recent vote" : undefined}
                                >
                                    {/* avatar */}
                                    {v.user?.image ? (
                                    ...
                                    <div className="min-w-0 flex-1">
                                        <div className="text-sm text-zinc-200 truncate">
                                            ...
                                            {v.value > 0 ? (
                                                <span className="text-emerald-400">upvoted</span>
                                            ) : (
